 ...
 */
export async function getConfigValue(key: keyof KalpanaConfig): Promise<string | undefined> {
 ...
 */
export async function setConfigValue(key: keyof KalpanaConfig, value: string): Promise<void> {
  const config = await loadConfig();
  config[key] = value;
 ...
 */
export async function removeConfigValue(key: keyof KalpanaConfig): Promise<void> {
  const config = await loadConfig();
  delete config[key];
  await saveConfig(config);
}
...
export async function validateConfig(): Promise<{ valid: boolean; missing: string[] }> {
  const config = await loadConfig();
  const required = ['OPENROUTER_API_KEY','MODEL_ID'];
  const missing: string[] = [];
  
  required.forEach(key => {
    if (!config[key as keyof KalpanaConfig] && !process.env[key]) {
      missing.push(key);
    }
  });
...
export async function displayConfig(): Promise<void> {
  ...
  if (Object.keys(config).length === 0) {
    console.log(chalk.yellow('⚠️  No configuration found. Run "kalpana config setup" to get started.'));
    return;
  }
...
export async function setupConfig(): Promise<void> {
  ...
  console.log('');
  console.log('This wizard will help you configure Kalpana with your API keys and preferences.');
  ...
  try {
    ...
    // Required settings
    console.log(chalk.yellow('📋 Required Settings:'));
    config.OPENROUTER_API_KEY = await question('OpenRouter API Key (required): ');
    
    if (!config.OPENROUTER_API_KEY?.trim()) {
      console.log(chalk.red('❌ OpenRouter API Key is required. Setup cancelled.'));
      rl.close();
      return;
    }
    ...
    const hyperBrowserKey = await question('HyperBrowser API Key (for web automation): ');
    ...
    const context7Key = await question('Context7 API Key (for documentation search): ');
    if (context7Key.trim()) config.CONTEXT7_API_KEY = context7Key;
    
    const geminiKey = await question('Google Gemini API Key (for multi-modal analysis): ');
    ...
    if (googleClientSecret.trim()) config.GOOGLE_CLIENT_SECRET = googleClientSecret;
    
    console.log('');
    console.log(chalk.yellow('⚙️  Advanced Settings (Optional):'));
    const dockerHost = await question('Docker Host (leave empty for auto-detection): ');
    if (dockerHost.trim()) config.DOCKER_HOST = dockerHost;
    
    const sandboxPath = await question('Default Sandbox Path (leave empty for ./.sandbox): ');
    ...
    console.log(chalk.cyan('  kalpana config show') + ' - View current configuration');
    console.log(chalk.cyan('  kalpana config set <key> <value>') + ' - Update specific settings');
