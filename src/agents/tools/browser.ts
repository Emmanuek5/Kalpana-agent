...
export function buildBrowserTools() {
  return {
    ...
    "browser.navigateAndTakeScreenshot": tool<
      { url: string; path?: string; fullPage?: boolean; waitUntil?: 'load' | 'domcontentloaded' | 'networkidle0' | 'networkidle2'; timeout?: number; scrollWaitMs?: number },
      { success: boolean; title?: string; url?: string; screenshot?: string; path?: string; error?: string }
    >({
      description: "Navigate to a URL, auto-scroll to bottom to load dynamic content, then capture a screenshot (saved to sandbox if path is provided).",
      inputSchema: zodSchema(
        z.object({
      ...
      execute: createSafeToolWrapper("browser.navigateAndTakeScreenshot", navigateAndTakeScreenshot as any),
    }),
  } as const;
}
